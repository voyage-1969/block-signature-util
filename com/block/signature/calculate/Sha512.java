...
public class Sha512 {
    ...
    private static long RTR(long x, int y) {
        return (x >>> y) ^ (x << (64 - y));
    }

    private static long EP0(long x) {
        return RTR(x, 28) ^ RTR(x, 34) ^ RTR(x, 39);
    }

    private static long EP1(long x) {
        return RTR(x, 14) ^ RTR(x, 18) ^ RTR(x, 41);
    }

    private static long SIG0(long x) {
        return RTR(x, 1) ^ RTR(x, 8) ^ (x >>> 7);
    }

    private static long SIG1(long x) {
        return RTR(x, 19) ^ RTR(x, 61) ^ (x >>> 6);
    }
